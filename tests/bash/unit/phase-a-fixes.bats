...
setup() {
    ...
    PHASE_A="$PA_DIR/phase-a-setup.md $PA_DIR/phase-a-s2-discovery.md $PA_DIR/phase-a-s3-s5-specs.md $PA_DIR/phase-a-s6-s8-scaffold.md $PA_DIR/phase-a-s9-s10-review.md $FORGE_DIR/rules/universal-agent-loop.md"
}
...
@test "Phase A S6 stack check uses variable not literal {stack}" {
    # Should use {STACK_BACKEND} not literal {stack}
    run grep "STACK_BACKEND" $PHASE_A
    assert_success
}
